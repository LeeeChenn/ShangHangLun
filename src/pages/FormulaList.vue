...
const letterMap = ref<Record<string, Formula[]>>({});
const letters = computed(() => Object.keys(letterMap.value).sort());
...
watch(formulas, (newVal) => {
  letterMap.value = {};
  newVal.forEach((formula, index) => {
    const letter = formula.letter;
    if (!letterMap.value[letter]) {
      letterMap.value[letter] = [];
    }
    letterMap.value[letter].push({...formula, index});
  });
});
  ...
  <Card>
    ...
    <ul class="space-y-2 font-serif tracking-wide">
        <template v-for="letter in letters" :key="letter">
            ...
            <li v-for="(formula, index) in letterMap[letter]" :key="index" 
    ...
    <template #letter>
        <ul class="h-full flex flex-col justify-between rounded-gl">
            <li 
              ...
              :key="letter"
              class="text-center text-gray-500 hover:text-white hover:bg-blue-500 cursor-pointer text-sm px-2 py-1.5 rounded-lg transition-colors"
